...
<div
  class="js-cart-drawer fixed inset-0 z-90"
  data-state="closed"
>
  ...
  {# Drawer panel #}
  <div class="js-cart-drawer-panel absolute top-0 right-0 bottom-0 w-full max-w-105 flex flex-col bg-bg text-secondary">
    ...
    <div class="flex items-center justify-between px-5 py-4">
      ...
      <button
        type="button"
        class="js-cart-drawer-close cursor-pointer text-secondary bg-transparent border-0 p-1"
    ...
    <form action="{{ store.cart_url }}" method="post" class="js-ajax-cart-panel flex-1 flex flex-col min-h-0" data-store="cart-form" data-component="cart">
          ...
          {% for item in cart.items %}
            ...
            <div class="js-cart-item flex gap-3 py-3 border-b border-secondary/6" data-item-id="{{ item.id }}" data-store="cart-item-{{ item.product.id }}" data-component="cart.line-item">
              ...
              <div class="flex-1 min-w-0 flex flex-col gap-1">
                <a href="{{ item.url }}" class="text-[0.8125rem] leading-tight text-secondary no-underline line-clamp-2" data-component="name.short-name">
                  {{ item.short_name }}
                </a>
                ...
                <div class="flex items-center justify-between mt-auto">
                  ...
                  <div class="flex items-center border border-secondary/12 rounded-md overflow-hidden">
                    <span
                      class="js-cart-quantity-btn w-7 h-7 flex items-center justify-center bg-transparent cursor-pointer text-secondary"
                    ...
                    <input
                      ...
                      value="{{ item.quantity }}"
                      class="js-cart-quantity-input w-8 text-center text-[0.8125rem] font-medium text-secondary bg-transparent border-0 p-0 outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                  ...
                  <span class="js-cart-item-subtotal text-[0.8125rem] font-bold text-secondary" data-line-item-id="{{ item.id }}" data-component="subtotal.value" data-component-value="{{ item.subtotal | money }}">
                    {{ item.subtotal | money }}
                  </span>
                </div>
              </div>
              ...
              <button
                type="button"
                class="js-cart-remove-btn shrink-0 self-start cursor-pointer text-secondary opacity-50 bg-transparent border-0 p-0.5"
      ...
      <div class="js-empty-ajax-cart flex flex-col items-center justify-center h-full gap-4 text-center px-5" {% if cart.items %}style="display:none"{% endif %}>
        <i data-lucide="shopping-cart" class="w-12 h-12 opacity-30 text-secondary"></i>
        <p class="text-sm text-secondary opacity-60 m-0">
          {{ "Seu carrinho está vazio" | translate }}
        </p>
        <a href="{{ store.products_url }}" class="text-sm text-secondary underline">
      ...
      <div class="js-cart-drawer-footer js-visible-on-cart-filled sticky bottom-0 px-5 py-4" {% if not cart.items %}style="display:none"{% endif %}>
        <div class="flex justify-between items-center mb-3">
          ...
          <span class="js-ajax-cart-total js-cart-subtotal text-base font-bold text-secondary" data-priceraw="{{ cart.subtotal }}">
            {{ cart.subtotal | money }}
          </span>
        </div>
        <input
          ...
          class="js-cart-drawer-checkout block w-full py-3 text-center bg-secondary text-bg rounded-lg text-sm font-semibold no-underline transition-opacity duration-150 cursor-pointer border-0"
        />
      </div>
